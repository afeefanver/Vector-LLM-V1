...
# Decision prompts are large (stats JSON + RAG context) — need more time on CPU
ollama_client = OllamaClient(timeout=300)
...
class DecisionEngine:
    ...
    async def decide(self, query: str, csv_data: str = "") -> dict:
        ...
        # Trim stats to essentials only — smaller prompt = faster response on CPU
        slim_stats = {
            k: stats[k] for k in ("row_count", "columns", "trend", "group_summary")
            if k in stats
        }
        ...
        prompt = DECISION_PROMPT.format(
            query   = query.strip(),
            stats   = json.dumps(slim_stats, indent=2),
            context = context,
        )
    ...
    def _compute_stats(self, csv_data: str) -> dict:
        ...
        try:
            ...
            # Group-level aggregates — skip date/month columns, find true category
            if not cat.empty and not numeric.empty:
                kpi_col   = numeric.columns[-1]
                group_col = self._best_group_col(cat, df)
                if group_col:
    ...
    @staticmethod
    def _best_group_col(cat, df) -> str:
        """
        Pick the best categorical column for grouping.
        Skips month/date-like columns (many unique values matching calendar names).
        Prefers columns with 2-20 unique values — typical for region, product, category.
        """
        MONTH_NAMES = {
        ...
        for col in cat.columns:
            ...
            if n < 2 or n > 50:
                continue
            # Skip if values look like month names or dates
            sample = {str(v).lower().strip() for v in unique_vals[:6]}
            if sample & MONTH_NAMES:
                continue
            return col
        return None
