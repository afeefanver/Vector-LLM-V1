...
class RAGRetriever:
    ...
    def _chunk(text: str, size: int = 400, overlap: int = 80) -> list[str]:
        """
        Split text into overlapping chunks, with line-aware support for tabular CSV/data.

        For line-based content (CSV/JSONL), respects line breaks so rows are never sliced mid-row.
        For plain text, uses word sliding window.
        """
        lines = [line for line in text.splitlines() if line.strip()]

        # Line/CSV aware chunking if text has multiple newline-separated lines
        if len(lines) > 5 and "," in lines[0]:
            ...
            while i < len(data_rows):
                chunk_rows = data_rows[i : i + max_lines_per_chunk]
                chunk_text = header + "\n" + "\n".join(chunk_rows)
                chunks.append(chunk_text)
                i += max_lines_per_chunk - overlap_lines
                if i >= len(data_rows):
                    break
            return chunks if chunks else [text]

        # Word-based sliding window for general narrative documents
        words  = text.split()
        chunks = []
        i      = 0
